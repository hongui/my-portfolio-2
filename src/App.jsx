  ...
  const projects = [
    {
      ...
      sections: [{ title: "办公流程可视化设计", content: "通过梳理 20+ 项司法高频业务，我们将原本深藏在三级菜单下的功能提取至首屏。", image: "https://placehold.co/1200x750/f1f5f9/3b82f6?text=Justice+Workbench" }]
    },
    {
      id: 'p2',
      title: "法治舆情决策驾驶舱",
      ...
      sections: [{ title: "决策分析", content: "核心指标通过数字翻牌器动效增强仪式感。", image: "https://placehold.co/1200x750/f3e8ff/a855f7?text=Data+Dashboard" }]
    },
    {
      id: 'p3',
      title: "司法可视化大屏设计",
      ...
      sections: [{ title: "大屏动态视觉", content: "通过 3D 粒子建模与 AE 动画序列，构建具有司法威严感的动态背景。", image: "https://placehold.co/1200x750/ecfdf5/10b981?text=Smart+Justice+Screen" }]
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-100 selection:text-blue-900 overflow-x-hidden animate-in fade-in duration-500">
      ...
      <aside className="fixed left-0 top-0 h-full z-[100] group flex">
        <div className="h-full bg-white/80 backdrop-blur-xl border-r border-slate-100 flex flex-col items-center py-8 w-16 group-hover:w-48 transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] shadow-2xl shadow-slate-200/50 overflow-hidden">
          ...
          <div className="flex flex-col gap-4 w-full px-2">
            {navItems.map((item) => (
              <a 
                key={item.id}
                href={`#${item.id}`}
                className={`flex items-center h-12 rounded-xl transition-all duration-300 relative overflow-hidden group/item ${activeSection === item.id ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-900'}`}
              >
          ...
          <div className="mt-auto opacity-0 group-hover:opacity-100 transition-opacity pb-4">
            <p className="text-[8px] text-slate-300 font-black uppercase tracking-widest text-center">Portfolio v4.0</p>
          </div>
        </div>
      </aside>
      ...
      <main className="pl-16 md:pl-24 transition-all duration-500">
        ...
        {/* Hero Section */}
        <section className="relative pt-32 pb-24 px-6 max-w-7xl mx-auto flex flex-col justify-center min-h-[90vh]">
          <div 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
              ...
              <FadeIn delay={300}>
                <div className="bg-slate-900 text-white p-12 rounded-[3.5rem] relative overflow-hidden group shadow-2xl">
                  ...
                  <h3 className="text-2xl font-black mb-12 flex items-center gap-4">工具集 / Tooling</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                    {["Figma / MasterGo", "Axure / Prototyping", "UI Motion / AE", "Design Systems"].map((skill, i) => (
                      <div key={i} className="group/item">
                        <p className="text-xl font-black group-hover/item:translate-x-1 transition-transform">{skill}</p>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <section id="work" className="py-32 bg-slate-50 scroll-mt-20">
          <div className="max-w-7xl mx-auto px-6">
            <FadeIn className="mb-20">
              <h2 className="text-5xl font-black tracking-tighter mb-4 text-center">核心项目拆解 / Projects</h2>
            </FadeIn>
            ...
            <FadeIn>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 items-center">
                <div className="flex flex-col items-center gap-6">
                  ...
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">微信: PMR37</p>
                </div>
